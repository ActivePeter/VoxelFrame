	...
	namespace _Game {
		namespace _Physics {
			bool raycast2Triangle(
				...
				const Type::Vec3F& direction, const Type::Vec3F& triP1, const Type::Vec3F& triP2,
				const Type::Vec3F& triP3,
				...
				while (curX != endX || curY != endY || curZ != endZ) {
					...
					if (chunk) {
						...
						if (blockInfo.isEmptyBlock()) {
							...
							//if (hasColid)
							{
								return_blockPosition = { curX, curY, curZ };
